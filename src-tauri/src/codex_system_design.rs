...
fn schema() -> &'static str {
    r#"{"type":"object","additionalProperties":false,"required":["schemaVersion","title","summary","viewType","architecturePattern","activeC4Level","nodes","edges"],"properties":{"schemaVersion":{"type":"integer","const":1},"title":{"type":"string","minLength":1},"summary":{"type":"string"},"viewType":{"type":"string","enum":["structural","runtime","deployment","development"]},"architecturePattern":{"type":"string","enum":["auto","layered","hub_spoke","pipeline","event_driven","deployment"]},"activeC4Level":{"type":"string","enum":["context","container","component","code"]},"nodes":{"type":"array","maxItems":80,"items":{"type":"object","additionalProperties":false,"required":["id","type","name","description","technology","deployment","status","linkedFeatureIds","linkedUserFlowIds","linkedWireframeIds","codePaths","testPaths","configuration","c4Level","position","size"],"properties":{"id":{"type":"string","pattern":"^[A-Za-z0-9._-]+$"},"type":{"type":"string","enum":["client","service","database","cache","queue","external","component","group"]},"name":{"type":"string","minLength":1},"description":{"type":"string"},"technology":{"type":"string"},"deployment":{"type":"string"},"status":{"type":"string","enum":["planned","active","deprecated"]},"linkedFeatureIds":{"type":"array","items":{"type":"string"}},"linkedUserFlowIds":{"type":"array","items":{"type":"string"}},"linkedWireframeIds":{"type":"array","items":{"type":"string"}},"codePaths":{"type":"array","items":{"type":"string"}},"testPaths":{"type":"array","items":{"type":"string"}},"configuration":{"type":"string"},"c4Level":{"type":"string","enum":["context","container","component","code"]},"parentId":{"type":"string"},"position":{"type":"object","additionalProperties":false,"required":["x","y"],"properties":{"x":{"type":"number","minimum":-100000,"maximum":100000},"y":{"type":"number","minimum":-100000,"maximum":100000}}},"size":{"type":"object","additionalProperties":false,"required":["width","height"],"properties":{"width":{"type":"number","exclusiveMinimum":0,"maximum":2000},"height":{"type":"number","exclusiveMinimum":0,"maximum":2000}}}}}},"edges":{"type":"array","maxItems":160,"items":{"type":"object","additionalProperties":false,"required":["id","source","target","type","protocol","dataFormat","isAsync","authentication","errorHandling","description"],"properties":{"id":{"type":"string","pattern":"^[A-Za-z0-9._-]+$"},"source":{"type":"string"},"target":{"type":"string"},"type":{"type":"string","enum":["http","ipc","database_query","event","file","dependency"]},"protocol":{"type":"string"},"dataFormat":{"type":"string"},"isAsync":{"type":"boolean"},"sequence":{"type":"integer","minimum":1},"authentication":{"type":"string"},"errorHandling":{"type":"string"},"description":{"type":"string"}}}}}}"#
}
#[tauri::command]
pub async fn generate_system_design_with_codex(
    ...
    let prompt=format!("ProjectStudio의 시스템 설계 변경안을 JSON으로 작성하라. 먼저 PRD와 기능명세에서 architectural drivers, quality attributes, constraints, 주요 시나리오를 추출하고 후보 구조와 trade-off를 검토한 뒤 가장 타당한 구조를 제안하라. 각 노드에 C4 c4Level(context, container, component, code)을 지정하고 컨테이너 내부 요소는 parentId로 상위 경계를 연결하라. 런타임 시나리오의 연결은 sequence를 지정하라. viewType은 structural을 기본으로 하고 architecturePattern은 그래프 구조에 맞춰 auto, layered, hub_spoke, pipeline, event_driven, deployment 중 하나를 선택하라. 요구사항 관계를 추측으로 확정하지 말고 근거가 약한 연결은 summary와 configuration에 검토 필요로 기록하라. 제공된 안정 ID만 linkedFeatureIds와 linkedUserFlowIds에 사용한다. 현재 설계를 직접 수정하지 말고 완전한 제안 snapshot을 반환한다. 자기 연결·중복 연결·존재하지 않는 노드 연결을 만들지 않는다. 컴포넌트는 좌→우로 읽히게 배치하고 통신 프로토콜·인증·오류 처리를 구체적으로 작성한다. 연결된 저장소가 작업 디렉터리라면 기존 모듈·기술 스택·테스트 경로를 읽기 전용으로 확인해 codePaths와 testPaths에 반영한다. 프로젝트: {}. 저장소: {}. 추가 요청: {}\n\nPRD:\n{}\n\n현재 설계:\n{}\n\n기능명세:\n{}\n\n유저플로우:\n{}",input.project_name,repository.as_ref().map_or("연결 안 됨".into(),|path|path.to_string_lossy().into_owned()),if input.additional_request.trim().is_empty(){"없음"}else{input.additional_request.trim()},prd.chars().take(40_000).collect::<String>(),current,items(&input.features),items(&input.user_flows));
