...
새 프로젝트는 웹 서비스, 모바일 앱, 데스크톱 프로그램, 백엔드·CLI·자동화, 머신러닝, 데이터 분석, 기타 소프트웨어 중 유형을 먼저 선택합니다. 선택값은 SQLite 프로젝트 원본에 저장되고 PRD·기능명세·작업 흐름·시스템 설계와 품질 검사의 기준이 됩니다. 화면형 제품은 유저플로우와 와이어프레임을 사용하고, 비화면형 프로젝트는 실행 파이프라인을 사용하며 와이어프레임 단계를 생략합니다.
