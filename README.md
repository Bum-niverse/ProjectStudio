...
새 프로젝트는 로컬 규칙으로 PRD를 즉시 저장한 뒤, Codex CLI가 연결되어 있으면 해당 PRD를 구조화 입력으로 사용해 기능명세·유저플로우·시스템 설계를 자동 생성합니다. 생성 결과는 최소 상세도와 ID·연결 무결성을 검사한 뒤 SQLite에 저장하며, CLI 미설치·시간 초과·형식 오류가 발생해도 PRD는 유지됩니다. PRD 화면에는 Codex로 전달되는 범위와 생성 상태·결과 개수를 표시합니다.
...
3단계 기능명세 트리는 전체 구조와 대주제별 시트를 전환할 수 있습니다. 대기능에서 화면·하위 기능·사용자 동작과 검증 규칙이 형제 노드로 자연스럽게 분기되며, 직교 곡선 연결선으로 계층을 추적합니다.

4단계 유저플로우는 기능명세를 그대로 펼치지 않습니다. 사용자 역할과 목표별로 시작, 실제 화면, 구체적인 클릭·입력, 조건 분기, 사용자에게 보이는 성공·실패 피드백과 복구·완료만 가로 스윔레인에 표시합니다. 내부 RLS·데이터베이스·캐시·토큰 처리는 기능명세 수용 기준이나 시스템 설계에 남깁니다.
