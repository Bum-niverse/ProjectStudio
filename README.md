...
4단계 유저플로우는 기능명세를 그대로 펼치지 않습니다. 사용자 역할과 목표별로 서비스 진입, 현재 화면, 구체적인 클릭·입력, 다음 화면과 완료 결과만 가로 스윔레인에 표시합니다. 회원가입 방식처럼 사용자의 선택에 따라 다음 화면이 달라질 때만 여러 경로로 분기합니다. 내부 RLS·데이터베이스·URL 안전 검증·캐시·토큰 처리는 기능명세 수용 기준이나 시스템 설계에 남깁니다.
