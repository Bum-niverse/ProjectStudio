...
전체 흐름은 `프로젝트 → PRD → 기능명세 → 유저플로우 → 시스템 설계 → 품질 검사 → 내보내기`다. 프로젝트 유형에 따라 단계 이름과 검토 규칙이 달라진다.
...
- 오른쪽 상세: 노드 메타데이터와 기능·유저플로우·코드·테스트 링크, 연결 프로토콜·인증·오류 처리 편집
...
- 노드: 안정 ID, 유형, 이름, 설명, 기술, 배포 위치, 상태, 기능·유저플로우 ID, 코드·테스트 경로, 설정, 위치와 크기
...
`linkedWireframeIds`와 `wireframe_pages`는 2026-07-14 이전 작업공간을 열기 위한 읽기 호환 데이터다. 현재 화면과 생성 흐름은 해당 값을 새로 만들거나 편집하지 않는다.
...
포함: 기본 노드·연결 편집, 위치 복원, 그룹 동반 이동, SQLite 리비전, 기능·유저플로우·코드·테스트 링크, Codex 제안 승인·거절, 규칙 검토, Markdown·JSON·PDF·Mermaid 출력.
