...
export function UserFlowNodeCard({data,selected}:NodeProps){
  ...
  return <div className={`user-flow-node kind-${node.kind} color-${node.colorKey??"violet"} ${node.isCompleted?"is-completed":""}`} onClick={()=>nodeData.onSelect?.(node)}>
    ...
    <Handle className="flow-handle" type="target" position={Position.Left}/><span className="flow-node-kind"><i aria-hidden="true">{KIND_ICON[node.kind]}</i>{KIND_LABEL[node.kind]}</span><strong>{node.title}</strong><label className="flow-node-completion" onClick={event=>event.stopPropagation()} onPointerDown={event=>event.stopPropagation()}><input aria-label={`${node.title} 개발 완료`} checked={Boolean(node.isCompleted)} onChange={event=>nodeData.onCompletionChange?.(node,event.currentTarget.checked)} type="checkbox"/><span>{node.isCompleted?"완료":"미완료"}</span></label><Handle className="flow-handle" type="source" position={Position.Right}/>
  </div>;
}
