...
export type UserFlowNodeData={node:UserFlowNode;onSelect?:(node:UserFlowNode)=>void;onAdd?:(node:UserFlowNode)=>void;onEdit?:(node:UserFlowNode)=>void;onDelete?:(node:UserFlowNode)=>void};
...
export function UserFlowNodeCard({data,selected}:NodeProps){
  ...
  return <div className={`user-flow-node kind-${node.kind} color-${node.colorKey??"violet"}`} onClick={()=>nodeData.onSelect?.(node)}>
    ...
    <Handle className="flow-handle" type="target" position={Position.Left}/><span className="flow-node-kind"><i aria-hidden="true">{KIND_ICON[node.kind]}</i>{KIND_LABEL[node.kind]}</span><strong>{node.title}</strong><Handle className="flow-handle" type="source" position={Position.Right}/>
  </div>;
}
