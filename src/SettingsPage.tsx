...
interface DeveloperToolsStatus { claude: ToolStatus; codex: ToolStatus; antigravity: ToolStatus; git: ToolStatus; githubCli: ToolStatus; isGithubAuthenticated: boolean }
...
export function SettingsPage({ theme, projectId, onThemeChange, onClose }: SettingsPageProps) {
  ...
  async function handleCheckTools() {
    ...
    try {
      setTools(isTauri() ? await invoke<DeveloperToolsStatus>("check_developer_tools") : {
        claude: { isInstalled: false }, codex: { isInstalled: false }, antigravity: { isInstalled: false }, git: { isInstalled: false }, githubCli: { isInstalled: false }, isGithubAuthenticated: false,
      });
  ...
  return <section className="settings-page full-page">
    ...
    <section className="settings-section"><div className="settings-section-heading"><div><h3>도구 상태 확인</h3><p>LLM 및 각종 API와 `.projectstudio` 문서를 사용합니다.</p></div><button aria-label="도구 상태 다시 확인" className="tool-retry-button" onClick={() => void handleCheckTools()} disabled={isChecking} title={isChecking ? "확인 중" : "도구 상태 다시 확인"} type="button"><span aria-hidden="true">↻</span></button></div><div className="connection-status"><span>Claude CLI <strong>{status(tools?.claude)}</strong></span><span>Codex CLI <strong>{status(tools?.codex)}</strong></span><span>Antigravity <strong>{status(tools?.antigravity)}</strong></span><span>Git <strong>{status(tools?.git)}</strong></span><span>GitHub CLI <strong>{status(tools?.githubCli)}</strong></span><span>GitHub 인증 <strong>{authenticationStatus}</strong></span></div></section>
