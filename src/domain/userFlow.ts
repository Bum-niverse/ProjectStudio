...
function actionTitle(title:string):string{
  const rules:Array<[RegExp,string]>=[[/로그인|회원가입/,"로그인 정보 제출"],[/랜딩|서비스 진입/,"서비스 시작 선택"],[/프로필/,"프로필 저장"],[/장소 검색|좌표 입력/,"장소 선택"],[/지구본/,"지도 탐색"],[/핀 선택/,"플레이리스트 핀 열기"],[/위치 지정/,"이 위치로 지정"],[/제목|설명|커버/,"기본 정보 저장"],[/곡과|링크 추가/,"곡 링크 추가"],[/공개 범위|발행/,"플레이리스트 발행"],[/상세|곡 목록/,"상세 화면 열기"],[/Spotify|YouTube/,"외부 앱에서 듣기"],[/필터/,"필터 적용"],[/수정|삭제/,"변경 내용 저장"],[/신고$/,"신고 제출"],[/신고 검토/,"처리 상태 저장"]];
...
export function createUserFlowSpec(projectId:string,features:FeatureSpec[]):UserFlowSpec{
  ...
  requirements.forEach((requirement,laneIndex)=>{const laneY=laneIndex*laneHeight;const colorKey=FLOW_COLORS[laneIndex%FLOW_COLORS.length];const children=features.filter(feature=>feature.parentId===requirement.id).sort((a,b)=>a.sortOrder-b.sortOrder);const phaseId=`flow-${requirement.id}`;
