...
function createGlobeatFeatureSpec(projectId: string): FeatureSpec[] {
  const root=`${projectId}-root`;const definitions:FeatureSpec[]=[
    ...
    {id:`${projectId}-trip-route`,parentId:`${projectId}-trip-memory`,title:"일차별 여행 동선 저장",status:"done",priority:"high",role:"여행자",description:"방문지마다 일차와 플레이리스트를 지정해 순서 있는 비공개 여행으로 저장한다.",sortOrder:46,acceptanceCriteria:criteria(`${projectId}-trip-route`,["도쿄 1일차 시부야, 2일차 센소지, 3일차 오다이바를 저장하고 3개 장소·3일로 재조회한다.","오다이바 방문지에 기존 플레이리스트를 연결해 다시 열어도 선택이 유지된다."])},
    {id:`${projectId}-trip-photos`,parentId:`${projectId}-trip-memory`,title:"여행 사진과 대표 커버",status:"in_progress",priority:"high",role:"여행자",description:"JPEG·PNG·WebP 사진을 비공개 Storage에 저장하고 대표사진을 여행과 연결 플레이리스트의 시각적 표지로 사용한다.",sortOrder:47,acceptanceCriteria:criteria(`${projectId}-trip-photos`,["사진당 5MB, 여행당 12장 제한과 MIME 형식을 검증한다.","한 여행에 대표사진은 하나만 유지하고 소유자 폴더 정책을 적용한다.","실제 운영 브라우저에서 로컬 사진 업로드와 재조회까지 확인한다."])},
