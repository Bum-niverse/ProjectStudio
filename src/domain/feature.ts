...
function createGlobeatFeatureSpec(projectId: string): FeatureSpec[] {
  const root=`${projectId}-root`;const definitions:FeatureSpec[]=[
    ...
    {id:`${projectId}-spotify-oauth`,parentId:`${projectId}-tracks`,title:"Spotify 계정 보안 연결",status:"in_progress",priority:"critical",role:"큐레이터",description:"공식 Authorization Code와 PKCE로 Spotify 계정을 연결한다. state와 단기 HttpOnly 쿠키를 검증하고 외부 토큰은 서버에서 AES-256-GCM으로 암호화해 일반 사용자에게 조회 권한이 없는 저장소에 보관한다.",sortOrder:24,acceptanceCriteria:criteria(`${projectId}-spotify-oauth`,["연결 시작 전에 Globeat 로그인 토큰을 서버에서 다시 검증한다.","콜백의 state와 PKCE verifier가 일치하지 않거나 10분을 넘으면 연결을 거부한다.","access token과 refresh token은 브라우저·로그·ProjectStudio에 노출하지 않는다.","연결 해제 시 서버에 저장된 토큰을 삭제하고 다시 동의하기 전에는 API를 호출하지 않는다.","운영 Client ID·Secret이 없으면 연결 가능 상태로 표시하지 않는다."])},
