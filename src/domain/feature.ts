...
function createGlobeatFeatureSpec(projectId: string): FeatureSpec[] {
  const root=`${projectId}-root`;const definitions:FeatureSpec[]=[
    ...
    {id:`${projectId}-auth`,parentId:`${projectId}-onboarding`,title:"이메일·소셜 회원가입과 로그인",status:"ready",priority:"high",role:"사용자",description:"이메일 또는 소셜 계정으로 인증하고 세션을 유지한다.",sortOrder:3,acceptanceCriteria:criteria(`${projectId}-auth`,["회원가입·로그인·로그아웃과 오류 복구를 지원한다."])},
    ...
    {id:`${projectId}-pin-preview`,parentId:`${projectId}-explore`,title:"핀 선택과 플레이리스트 미리보기",status:"planned",priority:"critical",role:"여행자",description:"핀을 선택해 장소의 공개 플레이리스트 미리보기를 연다.",sortOrder:13,acceptanceCriteria:criteria(`${projectId}-pin-preview`,["같은 장소의 여러 플레이리스트를 탐색할 수 있다."])},
