...
export type ProjectType="app"|"machine_learning"|"data_analysis"|"general";
...
export interface PlanningArtifacts{prdMarkdown:string;features:FeatureSpec[];userFlow:{nodes:UserFlowNode[];edges:UserFlowEdge[]};systemDesign:SystemDesignSnapshot}

const TYPE_LABELS:Record<ProjectType,string>={app:"웹·앱",machine_learning:"머신러닝",data_analysis:"데이터 분석",general:"일반 소프트웨어"};
export function classifyProject(prdMarkdown:string,features:FeatureSpec[]):ProjectType{
  ...
  if(/웹|앱|모바일|화면|로그인|회원가입|사용자|버튼|페이지|서비스/.test(text))return"app";
  return"general";
}

export function inspectPlanningQuality(artifacts:PlanningArtifacts):PlanningQualityReport{
  const projectType=classifyProject(artifacts.prdMarkdown,artifacts.features);const findings:QualityFinding[]=[];let checks=0;let passed=0;
  ...
  check(artifacts.userFlow.nodes.some(node=>node.kind==="screen")&&artifacts.userFlow.nodes.some(node=>node.kind==="action")&&artifacts.userFlow.nodes.some(node=>node.kind==="result"),{id:"flow-contract",severity:projectType==="app"?"error":"warning",artifact:"user_flow",title:"화면·행동·결과 흐름이 완전하지 않습니다.",detail:"유저플로우의 필수 사용자 가시 단계가 누락됐습니다.",recommendation:"현재 화면 → 구체적인 사용자 행동 → 다음 화면 또는 결과를 연결하세요."});
  check(!artifacts.userFlow.nodes.some(node=>/RLS|SQL|데이터베이스 저장|토큰 갱신|URL 안전 검증|내부 처리/.test(`${node.title} ${node.description}`)),{id:"flow-internals",severity:"error",artifact:"user_flow",title:"유저플로우에 내부 구현이 섞였습니다.",detail:"사용자가 직접 수행하거나 확인할 수 없는 단계가 있습니다.",recommendation:"내부 처리는 기능 수용 기준이나 시스템 설계로 이동하세요."});
  ...
  return{projectType,projectTypeLabel:TYPE_LABELS[projectType],score,gate,checks,passedChecks:passed,findings};
}
