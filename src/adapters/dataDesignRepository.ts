...
export interface DataDesignRepository { initialize(projectId: string, initialSnapshot?: DataDesignSnapshot): Promise<DataDesignRevision>; save(projectId: string, current: DataDesignRevision, snapshot: DataDesignSnapshot): Promise<DataDesignRevision> }

class TauriRepository implements DataDesignRepository {
  async initialize(projectId: string, initialSnapshot = initialDataDesign()) { return parse(await invoke<StoredRevision>("initialize_data_design", { input: { projectId, revisionId: crypto.randomUUID(), contentJson: JSON.stringify(initialSnapshot), createdAt: new Date().toISOString() } })); }
...
class BrowserRepository implements DataDesignRepository {
  async initialize(projectId: string, initialSnapshot = initialDataDesign()) { const stored = localStorage.getItem(key(projectId)); if (stored) { const parsed = JSON.parse(stored) as DataDesignRevision; return { ...parsed, snapshot: normalizeDataDesign(parsed.snapshot) }; } const value = { id: crypto.randomUUID(), projectId, revisionNumber: 1, snapshot: initialSnapshot, createdAt: new Date().toISOString() }; localStorage.setItem(key(projectId), JSON.stringify(value)); return value; }
